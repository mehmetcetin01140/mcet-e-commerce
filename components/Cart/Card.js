...
import {addDoc,updateDoc,doc,collection,serverTimestamp,query,where,onSnapshot, deleteDoc} from "firebase/firestore"
  ...
  const increaseOrDecreaseQuantity = (id,increaseOrDecrease) =>{
    const ref = collection(db,'cart');
        ...
        if(findProduct){
            ...
            const newQuantity = {product:{...findProduct.product,quantity:findProduct.product.quantity+=increaseOrDecrease}}
            updateDoc(refe,newQuantity)
         
       }
       else{
      addDoc(ref,{findProduct:{...findProduct.product,quantity:1},user:currentUser.email})
       }
  }
  return (
