...
import {
  orderAmount,
  tax,
  decimalSupportedTotalPrice,
} from "../../helpers/DecimalManipulator";
        ...
        {currentUser ? (
          <>
            ...
            <Col lg={3} md={12} className="cart-complete">
              <h5>Sipariş Özeti</h5>
              <span>
                Sipariş Tutarı {`(${cartItems.length} ürün)`}:{" "}
                {orderAmount((totalPrice() - totalPrice() * 0.18).toFixed(0))}{" "}
                TL
              </span>
              <br />
              <span>
                KDV {"(%18)"}: {tax((totalPrice() * 0.18).toFixed(0))} TL
              </span>
              <br />
              ...
              <h5>
                {" "}
                Toplam Tutar : {decimalSupportedTotalPrice(totalPrice)} TL
              </h5>
